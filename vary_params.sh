...
function run_tao() {
    ...
    do
        ...
        java -cp out mtree.tests.MTTest --algorithm microCluster --W ${w} --slide 500 --R 1.9 --k 50 --datafile tao.txt > ${dir_path}/w_tao_${w}.log
    done
    for s in ${slide_list[*]}
    do
        real_s=`echo "scale=0; ${s}*10000/1" | bc`
        echo "processing tao, vary slide, slide = ${real_s}"
        java -cp out mtree.tests.MTTest --algorithm microCluster --W 10000 --slide ${real_s} --R 1.9 --k 50 --datafile tao.txt > ${dir_path}/s_tao_${real_s}.log
    done
    for r in ${R_list[*]}
    do
        real_r=`echo "scale=3; ${r}*1.9" | bc`
        echo "processing tao, vary r, r = ${real_r}"
        java -cp out mtree.tests.MTTest --algorithm microCluster --W 10000 --slide 500 --R ${real_r} --k 50 --datafile tao.txt > ${dir_path}/r_tao_${real_r}.log
    done
    for k in ${K_list[*]}
    do
        echo "processing tao, vary k, k = ${k}"
        java -cp out mtree.tests.MTTest --algorithm microCluster --W 10000 --slide 500 --R 1.9 --k ${k} --datafile tao.txt > ${dir_path}/k_tao_${k}.log
    done
}
...
function run_gau() {
    ...
    do
        ...
        java -cp out mtree.tests.MTTest --algorithm microCluster --W 100000 --slide 5000 --R 0.028 --k ${k} --datafile gaussian.txt > ${dir_path}/k_gau_${k}.log
    done
}
...
function run_stk() {
    ...
    do
        ...
        java -cp out mtree.tests.MTTest --algorithm microCluster --W ${w} --slide 5000 --R 0.45 --k 50 --datafile stock.txt > ${dir_path}/w_stk_${w}.log
    done
    for s in ${slide_list[*]}
    do
        real_s=`echo "scale=0; ${s}*100000/1" | bc`
        echo "processing stk, vary slide, slide = ${real_s}"
        java -cp out mtree.tests.MTTest --algorithm microCluster --W 100000 --slide ${real_s} --R 0.45 --k 50 --datafile stock.txt > ${dir_path}/s_stk_${real_s}.log
    done
    for r in ${R_list[*]}
    do
        real_r=`echo "scale=4; ${r}*0.45" | bc`
